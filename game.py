...
def promote(side, time_control, time_w, time_b, screen, clock):
  while True:
    ...
    pygame.display.flip()
    clock.tick(60)
def game(screen, clock, turn = False, time_control = (-1, -1), position = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"):
                ...
                if selected != None:
                  if squares.count(mousesquare)==1:
                    if gameboard.board[selected[0]][selected[1]].type == "P" and (mousesquare[0] == 0 or mousesquare[0] == 7):
                      promo, time_w, time_b = promote(gameboard.turn, time_control, time_w, time_b, screen, clock)
                      if promo == None:
                        running = False
                      if time_control!=(-1, -1):
                        if gameboard.turn:
